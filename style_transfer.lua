...
-- Model options
-- cmd:option('-model', 'models/instance_norm/the_scream.t7')
cmd:option('-model', 's528.t7')
cmd:option('-image_size', 768)
cmd:option('-median_filter', 3)
cmd:option('-timing', 0)

-- Input / output options
cmd:option('-input_image', 'chicago.jpg')
...
local function main()
  ...
  local function run_image(in_path, out_path)
    ...
    --     outputTmp = layertemp:forward(localInput)
    --     localOutput = outputTmp:clone()
    --     localInput=nil
    --     collectgarbage()
    --     layertemp:clearState()
    --     localInput = localOutput
    --     print("do forward===========")
    --   end
    -- end
    -- local img_out = localOutput
